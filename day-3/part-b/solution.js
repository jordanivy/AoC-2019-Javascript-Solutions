function findStepsToIntersections(wire, knownIntersections) {
    
    var stepsToEachIntersection = [];
    var startPoint = [0, 0];
    var map = [startPoint];

    var stepCounter = 0;
    for (var j = 0; j < knownIntersections.length; j++){
        startPoint = [0, 0];
        map = [startPoint];
        stepsToEachIntersection.push('N/A');
        stepCounter = 0;

        for (var i = 0; i < wire.length; i++){
            var direction = wire[i].substring(0, 1);
            var moveUnits = parseInt(wire[i].substring(1, wire[i].length));

            switch (direction) {
                case 'R':
                    for(var x = 1; x <= moveUnits; x++){
                        map.push([startPoint[0], startPoint[1] + x]);
                        stepCounter++;

                        if (map[map.length - 1].toString() == knownIntersections[j].toString()){
                            stepsToEachIntersection[j] = stepCounter;
                            stepCounter = 0;
                            break;
                        }
                    }
                    startPoint = ([startPoint[0], startPoint[1] + moveUnits]);
                    break;
                case 'L':
                    for(var x = 1; x <= moveUnits; x++){
                        map.push([startPoint[0], startPoint[1] - x]);
                ...
                case 'U':
                    for(var x = 1; x <= moveUnits; x++){
                        map.push([startPoint[0] + x, startPoint[1]]);
                        stepCounter++;
                        if (map[map.length - 1].toString() == knownIntersections[j].toString()){
                            stepsToEachIntersection[j] = stepCounter;
                            stepCounter = 0;
                    ...
                    startPoint = ([startPoint[0] + moveUnits, startPoint[1]]);
                    break;
                case 'D':
                    for(var x = 1; x <= moveUnits; x++){
                        map.push([startPoint[0] - x, startPoint[1]]);
    ...
    return stepsToEachIntersection;
}
